...
Before beginning, I decided to setup the project in a way that I felt would be indicative of a real world situation/coding standards.  To keep both Rider's and my sanity, I adjusted the coding standard of the scripts, swapping the plentiful use of `public` fields to serialize objects in the inspector window to use serialized private fields instead, and adding a bit of organization. I also installed Unity's new `Input System`, and setup the inputs to match the controls as they were in the original repository.
...
For the latter, I looked to how asteroids handle their own destruction and creation of smaller asteroids, and now upgrades, and found that this would be an easy place to spawn the lightning as well. Because it was already calcuating in certain percentages upgrades vs another asteroid, this would then also work towards not creating too many lightning storms. However,the way that asteroid splits, upgrades and now lightning are handled/created would be better served in a separate class, instead of within the `Asteroid` class, but for sake of moving along quicker and getting features in and tested, I left it as is.
